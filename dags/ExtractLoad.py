...
import os
from utils.ETL_functions import extract_routes, fetch_destinations_for_variants
...
def push_to_big_query(destination_table, project_id, dataset_name, **kwargs):
...
push_routes_to_big_query_operator = PythonOperator(
    ...
    op_args=['routes', project_id, 'routes'],
    dag=dag,
)

# Task Definitions for Pushing Csv to BigQuery 
push_destiantions_to_big_query_operator = PythonOperator(
    task_id='push_destinations_to_big_query_task',
    python_callable=push_to_big_query,
    op_args=['destinations', project_id, 'destinations'],
    dag=dag,
)

# Task dependencies: This sets the order of task execution
api_routes_extraction_operator >>  push_routes_to_big_query_operator >> api_destinations_extraction_operator >> push_destiantions_to_big_query_operator